...
function copyTask() {
      var sourceFiles = ['./*.html', './*config.js', './*styles.css'];
...
//        ]));
 ...
 function exportTask() {
  ...
  .pipe(shell([
           `stitch-cli login --private-api-key=${process.env.STITCH_PRIVATE_API_KEY} --api-key=${process.env.STITCH_API_KEY} --yes`,
           `cd <%= file.path %> && stitch-cli export --include-hosting --yes --app-id=${process.env.STITCH_APP_ID} --output=./stitch`
     ]));
}
...
exports.export = series(cleanTask, exportTask);
